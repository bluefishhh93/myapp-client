...
import { FilePenIcon, TrashIcon, CalendarIcon, PenIcon, Delete } from "lucide-react";
...
export default async function PublishedPage() {
    ...
    const { userPosts } = await getUserBlogs({ userId: user.id, published: true });

    if (userPosts.length === 0) {
        return (
            <div className="text-center py-10">
                <p className="text-xl text-muted-foreground">No published posts found.</p>
            </div>
        );
    }
